Feature: PurchaseState
  ...
  Scenario Outline: POST PurchaseState Twice with same code - expect <responseCode> as response code
    ...
    * def count = db.readValue('select count(*) FROM purchase_state')
    ...
    * method POST
    * status <responseCode>
    * eval if(__num==1 && count != db.readValue('select count(*) FROM purchase_state')) karate.fail("purchase_state count values in DB are different")
